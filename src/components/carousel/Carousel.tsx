...
export default function Carousel({ items, interval = 6000 }: CarouselProps) {
  ...
  // 🔥 New state: autoplay starts only after click
  ...
  // ------------------- AUTO SLIDE -------------------
  useEffect(() => {
    if (!isActivated || isPaused) return; // 🔥 autoplay only after user activates
  ...
  }, [isActivated, isPaused, interval, items.length]);
  ...
  const nextSlide = () => {
    activate(); // 🔥 Activate autoplay after manual navigation
    setCurrent((prev) => (prev + 1) % items.length);
  };
  ...
  return (
    <div
      ...
      onTouchStart={(e) => {
        onTouchStart(e);
        activate(); // 🔥 activate on touch
      }}
      onTouchEnd={onTouchEnd}
      onClick={activate} // 🔥 activate on click
    >
      {/* ---------------- BACKGROUND IMAGE ---------------- */}
      <AnimatePresence>
        <motion.div
          key={slide.id}
          animate={{ opacity: 1, scale: 2.0 }} // cinematic zoom
          transition={{
            duration: 100,
            ease: "easeOut",
          }}
